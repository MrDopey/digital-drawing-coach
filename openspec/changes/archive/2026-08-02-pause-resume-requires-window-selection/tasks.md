...
- [x] 1.1 In `main_window.py` `_update_status()`, set `self._pause_btn.setEnabled(...)` and `self._tray_pause_action.setEnabled(...)` based on `self._capture.target is not None`, in addition to the existing label logic.
...
- [x] 1.3 In `_on_window_lost()`, ensure the disabled/paused state is applied (already calls `_update_status()`; confirm it now disables the control per 1.1).
- [x] 1.4 In `_open_app_selection()`, confirm the existing `_update_status()` call after `set_target()` re-enables the control (should fall out of 1.1 automatically).
...
- [x] 3.4 Run `uv run pytest` and confirm all tests pass. (196/199 pass; 3 pre-existing failures in `test_stuck_detector.py::test_cooldown_blocks_retrigger`, `test_manual_trigger_bypasses_cooldown`, and `test_diagnostics.py::test_dialog_llm_fail_row` reproduce in isolation on unmodified files, unrelated to this change.)
