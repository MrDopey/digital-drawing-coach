...
### 4. `meta.json` gains `name: str` (optional, defaults to timestamp label)

Reading code uses `meta.get("name") or default_name(start_time)`. Writing happens on session create and on explicit rename. Backward-compatible — old sessions without `name` show their timestamp label.
...
1. Deploy: existing sessions gain no `name` field — picker displays timestamp label, which is equivalent to today's behaviour.
