...
The system SHALL send the most recent screenshot and a configurable number of prior history frames (default: 2, range: 0–10, set in LLM settings) to the configured vision LLM and return structured drawing feedback. Images SHALL be encoded as base64 and sent via the LiteLLM `completion()` API. The returned `FeedbackResponse` SHALL include a `frame_hashes` field containing the SHA-256 hex digest of each frame's image data (in the same order as the frames sent), computed at request time.
...
- **WHEN** a successful LLM response is returned
