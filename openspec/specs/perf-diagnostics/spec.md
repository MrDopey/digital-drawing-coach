# perf-diagnostics Specification

## Purpose
Defines the opt-in performance instrumentation used to diagnose GUI-thread stalls: a heartbeat/watchdog pair that detects and reports event-loop blockages with sampled stacks, scoped probes that time suspect operations, live object and signal-receiver counts, garbage-collection pause reporting, a shutdown run summary, a user-facing perf snapshot, and a `faulthandler` fallback for GIL-pinned stalls. All of it is inert unless explicitly enabled.
