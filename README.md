...
- **Feedback Management** panel: opening it (toolbar button, tray menu) just shows the panel — no LLM request is sent until you click **Request Feedback** inside it (the hotkey and automatic stuck-detection still request feedback directly, as before). A left sidebar lists every feedback entry for the active session in reverse-chronological order (`DD Mon  HH:MM: <mode>`); clicking a row jumps to that entry and stays in sync with the **Previous**/**Next** buttons. Overlay-mode entries show the existing zoomable/pannable annotated image (top) alongside the feedback text (bottom) in a resizable split with a draggable divider; feedback text always scrolls to show the full response, and the overlay image can be zoomed in/out (`+`/`−`/**Reset** buttons, or `Ctrl+Wheel`) and panned to see annotation detail. Quick Hint, Full Critique, and Practice Exercise entries instead show a small thumbnail of the last frame sent — click it to open the full image in the system default viewer.
- Feedback is persisted to disk as it arrives (`feedback/<session-id>/` inside the session folder — JSON plus a thumbnail, and for overlay mode the full composited image) and reloaded into the sidebar on startup and whenever the active session changes, so history survives an app restart. **Request Feedback** is disabled with a tooltip ("Already generated for this drawing and mode") whenever the current frames and selected mode exactly match the last saved entry for that mode, and re-enables automatically once a new frame is captured or a different mode is selected.
...
Each session directory also has a `feedback/` subfolder (`sessions/<session-id>/feedback/`) holding one JSON file per feedback response (`YYYYMMDD_HHMMSS_<mode>.json`, plus a `..._thumb.jpg` thumbnail and, for overlay mode, a full-resolution `..._overlay.png`) — this is what the Feedback Management panel's sidebar reloads on startup and on session switch.
...
Both caps and the re-summarisation interval are editable from **Settings → Memory**. Use the **Memory** window (from the main window's button row) to review observations grouped by category — hovering over a row highlights its background, matching the history panel's row-hover behavior — delete individual entries, clear everything, or export both memory files to a folder of your choice. Its "Current coach's notes" preview (`MemoryStore.current_notes()`) is a read-only fetch of already-persisted state — opening or refreshing that window never triggers a re-summarisation LLM call or writes to `memory_summaries.json`; only an actual feedback request (`MemoryStore.summarise()`) can do that. The **Progress** window shows recurring themes and a timeline of past sessions.
